...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE sifa SET title = ?, description = ? WHERE id = ?");
    $success = $stmt->execute([$title, $description, $id]);

    // Handle video update if provided
    if (isset($_FILES['video']) && $_FILES['video']['error'] === 0) {
        $uploadDir = '../videos/sifa/';
        $oldVideo = $conn->query("SELECT video FROM sifa WHERE id = $id")->fetchColumn();
        ...
        if (move_uploaded_file($_FILES['video']['tmp_name'], $uploadPath)) {
            $conn->prepare("UPDATE sifa SET video = ? WHERE id = ?")->execute([$fileName, $id]);
        }
    }
