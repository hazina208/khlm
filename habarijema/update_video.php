...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("UPDATE habarijema SET title = ?, description = ? WHERE id = ?");
    $success = $stmt->execute([$title, $description, $id]);

    // Handle video update if provided
    if (isset($_FILES['video']) && $_FILES['video']['error'] === 0) {
        $uploadDir = '../videos/habarijema/';
        $oldVideo = $pdo->query("SELECT video FROM habarijema WHERE id = $id")->fetchColumn();
        ...
        if (move_uploaded_file($_FILES['video']['tmp_name'], $uploadPath)) {
            $pdo->prepare("UPDATE habarijema SET video = ? WHERE id = ?")->execute([$fileName, $id]);
        }
    }
